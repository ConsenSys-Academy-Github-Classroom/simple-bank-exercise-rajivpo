...
contract SimpleBank {
    ...
    function deposit() public payable returns (uint256) {
        ...
        require(enrolled[msg.sender], "Must be enrolled before depositing");
    ...
    function withdraw(uint256 withdrawAmount) public returns (uint256) {
        ...
        require(
            balances[to] >= withdrawAmount,
            "Withdraw amount must be <= deposited"
        );
        balances[to] -= withdrawAmount;
        address(to).transfer(withdrawAmount);
        emit LogWithdrawal(to, withdrawAmount, balances[to]);
        return balances[to];
    }
}
